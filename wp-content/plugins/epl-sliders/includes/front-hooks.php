...
class EPL_SLIDER_CONFIG {
	...
	public function slider_config_options() {
		...
		return array(
			...
			'$LazyLoading'			=> 'epl_slider_LazyLoading',
	...
	function epl_slider_render_options() {
		...
		$use_thumbs = $this->thumbs_on_mobile() ? 0 : $this->slide_get_option('epl_slider_use_thumbnails');
			...
			'$LazyLoading'			=> $this->slide_get_option('epl_slider_LazyLoading'),
	...
	function epl_slider_gallery() {
		...
		$src2 = $this->slide_get_option('epl_slider_enable_lazy') == 'true' ? 'src2' : 'src';
		...
		?>
			<div class="<?php echo $epl_slider_wrapper_class; ?>">
				...
				<div id="epl_slider_container_<?php echo $post->ID?>" class="epl_slider_container" <?php echo 'style="'.$inlinecss.'"';?> >
						...
						foreach ( $images as $attachment_id => $attachment ) {
							...
							if( ($width != 0 || $height != 0 ) && !$this->is_allowed_archives() ) {
								...
								$args   = apply_filters( 'epl_slider_crop_image_args',
									array(
										'position' => $this->slide_get_option('epl_slider_crop_alignment'),
										'width'    => $width,
										'height'   => $height,
										'quality'  => $this->slide_get_option('epl_slider_crop_quality'),
										'retina'   => false
									)
								);
								...
								if( $allow_crop) {
									$allow_crop = $this->slide_get_option('epl_slider_crop');
								}
							 ...
							 if($this->slide_get_option('epl_slider_use_thumbnails') == 2 && !$this->is_allowed_archives()) {
								 if($slider_thumb_height != 0 || $slider_thumb_height != 0) {
									...
									$thumb = $this->resize_image(
										...
										$args['width'],
										$args['height'],
										$this->slide_get_option('epl_slider_crop'),
										$args['position'],
										$args['quality'],
										$args['retina']
									);
							...
							if( is_wp_error($image) ||  is_wp_error($thumb) )
								continue;

							$thumb_html = '';
							if($this->slide_get_option('epl_slider_use_thumbnails') == 2 && !$this->is_allowed_archives()) {
								$thumb_html = "<img u=\"thumb\" $src2=\"{$thumb}\" />";
							}
					...
					<span u="arrowleft" class="epl-slider-left-nav" ><button class="paginate left"><i></i><i></i></button>
					</span>
					<span u="arrowright" class="epl-slider-right-nav" ><button class="paginate right"><i></i><i></i></button>
					</span>
