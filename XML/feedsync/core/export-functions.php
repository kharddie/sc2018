...
function feedsync_list_listing_type( $type = '', $status = '' ) {
    ...
    global $feedsync_db,$pagination;

    // instantiate the pagination object
    $pagination = new Zebra_Pagination();

    $query = "SELECT SQL_CALC_FOUND_ROWS * FROM feedsync WHERE 1 = 1 ";
    ...
    $query .= ' LIMIT ' . (($pagination->get_page() - 1) * $records_per_page) . ', ' . $records_per_page . '';

    $results    = $feedsync_db->get_results($query);
    $total_rows = $feedsync_db->get_results('SELECT FOUND_ROWS() AS rows');
